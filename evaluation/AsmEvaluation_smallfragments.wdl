...
workflow runDipcall {
    input {
        ...
        String gene_locus = "chr6:29,941,260-29,949,572"
        String region_locus = "chr6:25,726,063-33,410,226"
        Boolean too_short
    ...
    if (too_short) {
        call ReplaceSequences as hap1 { input:
            input_fasta = eval_hap1_fasta,
            reference_fasta = HG38_reference_fasta,
            output_file = "haplotype1",
            gene_locus = gene_locus,
            locus = region_locus,       
        }
        call ReplaceSequences as hap2 { input:
            input_fasta = eval_hap2_fasta,
            reference_fasta = HG38_reference_fasta,
            output_file = "haplotype2",
            gene_locus = gene_locus,
            locus = region_locus,       
        }
    ...
    call dipcall as eval_dipcall { input:
        assemblyFastaPat = select_first([hap1.output_fasta, eval_hap1_fasta]),
        assemblyFastaMat = select_first([hap2.output_fasta, eval_hap2_fasta]),
        referenceFasta = HG38_reference_fasta,
    }
...
task ReplaceSequences {
    input {
        ...
        Int disk_size_gb = ceil(size(reference_fasta, "GiB") + 10)
        Int mem_gb = 16
        Int cpu = 2
        Int preemptible = 1
    }
    ...
    runtime {
        docker: "hangsuunc/mhc_evaluation:v1"
        disks: "local-disk " + disk_size_gb + " HDD"
        memory: mem_gb + " GiB"
        cpu: cpu
        preemptible: preemptible
    ...
    output {
        File output_fasta = "~{output_file}.fa"
    }
}