...
pub fn start(output: &PathBuf, fasta_paths: &Vec<PathBuf>, seq_paths: &Vec<PathBuf>) {
    ...
    for seq_url in seq_urls {
        ...
        let progress_bar = skydive::utils::default_unbounded_progress_bar("Searching for similar reads (0 found)");
        ...
        reader.fetch(FetchDefinition::All).expect("Failed to fetch reads");
        let records: Vec<_> = reader
            ...
            .filter_map(|read| {
                ...
                if current_processed % UPDATE_FREQUENCY == 0 {
                    progress_bar.set_message(format!("Searching for similar reads ({} found)", found_items.load(Ordering::Relaxed)));
                ...
                if num_kmers > num_threshold {
                    ...
                    if current_found % UPDATE_FREQUENCY == 0 {
                        progress_bar.set_message(format!("Searching for similar reads ({} found)", (current_found + 1).to_formatted_string(&Locale::en)));
                    }
                    Some(read)
                } else {
                    None
        ...
        progress_bar.set_message(format!("Found {} reads in {} reads total.",
    ...
    for (i, record) in all_records.iter().enumerate() {
        ...
        writeln!(file, "{}", String::from_utf8_lossy(&record.seq().as_bytes())).expect("Could not write to file");
    }

    skydive::elog!("Wrote {} reads to {}.", all_records.len(), output.to_str().unwrap());
}
