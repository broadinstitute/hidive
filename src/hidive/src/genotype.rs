...
pub fn start(
    ...
    // Read VCF file
    let mut vcf_in = Reader::from_path(vcf_path).unwrap();

    let mut vcf_records: HashMap<String, HashMap<i64, rust_htslib::bcf::Record>> = HashMap::new();
    let header = vcf_in.header().clone();
    for record in vcf_in.records().flatten() {
        let contig_name =
            String::from_utf8_lossy(header.rid2name(record.rid().unwrap()).unwrap()).to_string();
        vcf_records
            .entry(contig_name)
    ...
    let mut vcf1 = Writer::from_path(output, &vcf_header1, true, Format::Vcf).unwrap();

    let vcf_header2 = initialize_vcf_header(&fasta, sample_name);
    let mut vcf2 = Writer::from_path(output, &vcf_header2, true, Format::Vcf).unwrap();
    ...
    for (chr, start, stop, name) in loci {
        ...
        let (matrix1, mloci1, read_map1) = prepare_matrix(&chr, start, stop, &mut bam, &fasta, 1);
        let (matrix2, mloci2, read_map2) = prepare_matrix(&chr, start, stop, &mut bam, &fasta, 2);
        ...
        skydive::elog!(" - phasing {} {}...", matrix1.len(), matrix2.len());

        // let hom_ref_gt = &[GenotypeAllele::Unphased(0), GenotypeAllele::Unphased(0)];

        /*
        let sites = mloci1.iter().chain(mloci2.iter()).collect::<BTreeSet<_>>();

        let mut h1 = Vec::new();
        let mut h2 = Vec::new();
        let mut loci = Vec::new();

        for (idx, &&loc) in sites.iter().enumerate() {
            skydive::elog!(" - {} {} {} {}", loc, idx, matrix1.len(), matrix2.len());

            // skydive::elog!(" - {} {} {:?} {:?} {:?} {:?}", idx, loc, mloci1.get(idx), mloci2.get(idx), matrix1.get(idx), matrix2.get(idx));

            if vcf_records.contains_key(&chr) && vcf_records[&chr].contains_key(&(loc as i64)) {
                let record = vcf_records[&chr][&(loc as i64)].clone();
                let gt = record.genotypes().unwrap().get(0);

                // skydive::elog!(" - {} {}", loc, gt);

                let ref_base = fasta.fetch_seq_string(&chr, usize::try_from(loc).unwrap(), usize::try_from(loc).unwrap()).unwrap();

                if matrix1.len() > idx {
                    let read_alleles_map1 = matrix1.get(idx).unwrap();
                    let best_allele1 = get_best_allele(read_alleles_map1);

                    if format!("{}", gt) != "0/0" || best_allele1.len() > 100 {
                        // skydive::elog!(" - {} {} {} best: {:?}", 1, idx, loc, best_allele1);

                        h1.push(Some(best_allele1));
                    } else {
                        // skydive::elog!(" - {} {} {} ref: {:?}", 1, idx, loc, ref_base);

                        h1.push(Some(ref_base.clone()));
                    }
                ...
                if matrix2.len() > idx {
                    let read_alleles_map2 = matrix2.get(idx).unwrap();
                    let best_allele2 = get_best_allele(read_alleles_map2);

                    if format!("{}", gt) != "0/0" || best_allele2.len() > 100 {
                        // skydive::elog!(" - {} {} {} best: {:?}", 2, idx, loc, best_allele2);

                        h2.push(Some(best_allele2));
                    } else {
                        // skydive::elog!(" - {} {} {} ref: {:?}", 2, idx, loc, ref_base);

                        h2.push(Some(ref_base.clone()));
                    }
        ...
        // let loci = mloci1.iter().chain(mloci2.iter()).cloned().collect::<Vec<_>>();
        add_variant_records(loci, h1, h2, &mut fasta, chr.clone(), start, stop, &mut vcf);
        // add_variant_records(mloci2, Vec::new(), h2, &mut fasta, chr.clone(), start, stop, &mut vcf);
        */
    }
}

fn get_best_allele(read_alleles_map1: &BTreeMap<usize, (String, u8)>) -> String {
    let mut seq_counts = std::collections::HashMap::new();
    let mut longest_seq = String::new();
    for (_, (seq, _)) in read_alleles_map1 {
        *seq_counts.entry(seq.clone()).or_insert(0) += 1;
        if seq.len() > longest_seq.len() {
            longest_seq = seq.clone();
        }
    }
    let most_frequent = seq_counts
        .iter()
        .max_by_key(|(_, count)| *count)
        .map(|(seq, _)| seq.clone())
        .unwrap_or_default();

    if longest_seq.len() > 1000 {
        longest_seq
    } else {
        most_frequent
    }
...
fn clean_variants(
    mloci: &Vec<u32>,
    h1: &Vec<Option<String>>,
    h2: &Vec<Option<String>>,
) -> (Vec<u32>, Vec<Option<String>>, Vec<Option<String>>) {
...
fn filter_variants(
    ...
    fasta: &rust_htslib::faidx::Reader,
) -> Vec<Option<String>> {
    ...
    for i in 0..variants.len() {
        if include[i] && filtered[i].is_some() && filtered[i].as_ref().unwrap().len() > 100 {
            for j in (i + 1)..variants.len() {
                if include[j] && filtered[j].is_some() {
                    ...
                    if (j_len as f64) >= (i_len as f64) * 0.5
                        && (j_len as f64) <= (i_len as f64) * 1.5
                    {
        ...
        if include[i] && filtered[i].is_some() && filtered[i].as_ref().unwrap().len() == 2 {
            let ref_pos = mloci[i];
            let prev_ref_str = fasta
                .fetch_seq_string(
                    chr,
                    usize::try_from(ref_pos - 2).unwrap(),
                    usize::try_from(ref_pos - 2).unwrap(),
                )
                .unwrap();
            let next_ref_str = fasta
                .fetch_seq_string(
                    chr,
            ...
            let first_allele_str = filtered[i]
                .as_ref()
                .unwrap()
                .chars()
                .next()
                .unwrap()
                .to_string();
            let last_allele_str = filtered[i]
                .as_ref()
                .unwrap()
                .chars()
                .last()
...
fn initialize_vcf_header(fasta: &rust_htslib::faidx::Reader, sample_name: &String) -> Header {
    ...
    for seq_name in fasta.seq_names().unwrap() {
        let header_contig_line = format!(
            "##contig=<ID={},length={}>",
            seq_name,
            fasta.fetch_seq_len(&seq_name)
        );
        vcf_header.push_record(header_contig_line.as_bytes());
    }

    let header_end_line =
        r#"##INFO=<ID=END,Number=1,Type=Integer,Description="Stop position of the interval">"#;
...
fn prepare_matrix_old(
    chr: &String,
    start: u64,
    stop: u64,
    mut bam: rust_htslib::bam::IndexedReader,
    fasta: &rust_htslib::faidx::Reader,
) -> (Vec<BTreeMap<usize, (String, u8)>>, Vec<u32>) {
    ...
    for p in rust_htslib::bam::Read::pileup(&mut bam) {
        let pileup = p.unwrap();
        ...
        let ref_str = fasta
            .fetch_seq_string(
                chr,
                usize::try_from(pileup.pos()).unwrap(),
                usize::try_from(pileup.pos()).unwrap(),
            )
            .unwrap();
        ...
        for alignment in pileup.alignments() {
            ...
            if !alignment.is_del() && !alignment.is_refskip() {
                ...
                allele_map.insert(
                    *read_ids.get(&qname).unwrap(),
                    (String::from_utf8_lossy(&seq).to_string(), q),
                );
            ...
            match alignment.indel() {
                ...
                rust_htslib::bam::pileup::Indel::Del(len) => {
                    ...
                    allele_map.insert(
                        *read_ids.get(&qname).unwrap(),
                        (String::from_utf8_lossy(&full_seq).to_string(), q),
                    );
...
fn prepare_matrix(
    ...
    Vec<BTreeMap<usize, (String, u8)>>,
    Vec<u32>,
    HashMap<usize, Record>,
) {
    //                            pos           read    allele  qual
    let mut alleles_map: BTreeMap<u32, BTreeMap<usize, (String, u8)>> = BTreeMap::new();
    let mut read_ids = HashMap::new();
    ...
    for (read_index, read) in rust_htslib::bam::Read::records(bam)
        .filter(|r| r.is_ok())
        .flatten()
        .enumerate()
    {
        // Skip reads that don't match the requested haplotype
        if let Ok(aux) = read.aux(b"HP") {
            if let record::Aux::U8(hp) = aux {
                if hp != haplotype {
                    continue;
                }
            }
        ...
        for ce in read.cigar().iter() {
            match ce {
                record::Cigar::Ins(len) => {
                    if start as u32 <= ref_pos + 1 && ref_pos + 1 < stop as u32 {
                        let ref_str = fasta
                            .fetch_seq_string(
                                chr,
                                usize::try_from(ref_pos - 1).unwrap(),
                                usize::try_from(ref_pos - 1).unwrap(),
                            )
                            .unwrap();
                        let ref_base = ref_str.as_bytes()[0];
                        ...
                        let q = *qual.iter().min().unwrap();
                        let seq =
                            [&[ref_base], &read.seq().as_bytes()[qpos_start..qpos_stop]].concat();
                        ...
                        if let Some((existing_seq, _)) = allele_map.get(&read_index) {
                            ...
                            allele_map
                                .insert(read_index, (String::from_utf8_lossy(&seq).to_string(), q));
                        }

                        read_ids.insert(
                            String::from_utf8_lossy(&read.qname()).to_string(),
                            read_index,
                        );
                ...
                record::Cigar::Del(len) => {
                    if start as u32 <= ref_pos + 1 && ref_pos + 1 < stop as u32 {
                        ...
                        let seq = vec![ref_base; 1]
                            .into_iter()
                            .chain(std::iter::repeat(b'-').take(*len as usize))
                            .collect::<Vec<_>>();

                        let allele_map =
                            alleles_map.entry(ref_pos - 1).or_insert_with(BTreeMap::new);
                ...
                record::Cigar::Match(len)
                | record::Cigar::Equal(len)
                | record::Cigar::Diff(len) => {
                    for i in 0..*len as usize {
                        if start as u32 <= ref_pos + i as u32 + 1
                            && ref_pos + i as u32 + 1 < stop as u32
                        {
                            let ref_str = fasta
                                .fetch_seq_string(
                                    chr,
                                    usize::try_from(ref_pos + i as u32).unwrap(),
                                    usize::try_from(ref_pos + i as u32).unwrap(),
                                )
                                .unwrap();
                            let ref_base = ref_str.as_bytes()[0];
                            ...
                            if ref_base != read_base {
                                ...
                                let allele_map = alleles_map
                                    .entry(ref_pos + i as u32)
                ...
                record::Cigar::SoftClip(len) => {
                    if start as u32 <= ref_pos + 1 && ref_pos + 1 < stop as u32 {
                        ...
                        // skydive::elog!(" - softclip {} {}", ref_pos, String::from_utf8_lossy(&seq));
                    }

                    read_pos += len;
                }
                record::Cigar::HardClip(_) => {}
                record::Cigar::RefSkip(len) => {
                    ref_pos += len;
                }
                record::Cigar::Pad(_) => {}
            }
    ...
    for p in rust_htslib::bam::Read::pileup(bam) {
        let pileup = p.unwrap();
        ...
        for alignment in pileup.alignments() {
            ...
            if alleles_map.contains_key(&(pileup.pos()))
                && !alleles_map
                    .get(&(pileup.pos()))
                    .unwrap()
                    .contains_key(&read_index)
            {
                ...
                let allele_map = alleles_map
                    .get_mut(&pileup.pos())
                    .expect("Position not found in alleles_map");
                allele_map.insert(read_index, (String::from_utf8_lossy(&seq).to_string(), q));
            }
        }

        // if alleles_map.contains_key(&pileup.pos()) {
        //     skydive::elog!("{} {:?}", pileup.pos(), alleles_map.get(&pileup.pos()).unwrap());
        // }
    }
...
fn add_variant_records(
    mloci: Vec<u32>,
    h1: Vec<Option<String>>,
    h2: Vec<Option<String>>,
    fasta: &mut rust_htslib::faidx::Reader,
    chr: String,
    start: u64,
    stop: u64,
    ...
    for pos in start as u32..stop as u32 {
        ...
        if hap_alleles.contains_key(&pos) {
            ...
            */
            ...
            if a1.contains("-") {
                let ref_full_str = fasta
                    .fetch_seq_string(
                        &chr,
                        usize::try_from(pos).unwrap(),
                        usize::try_from(pos + (a1.len() as u32) - 1).unwrap(),
                    )
                    .unwrap();
            ...
            if a2.contains("-") {
                let ref_full_str = fasta
                    .fetch_seq_string(
                        &chr,
                        usize::try_from(pos).unwrap(),
                        usize::try_from(pos + (a2.len() as u32) - 1).unwrap(),
                    )
                    .unwrap();
                if ref_full_str.len() > ref_allele.len() {
                    ref_allele = ref_full_str;
            ...
            let mut allele_set = alt_alleles
                .iter()
            ...
            let i1 = record
                .alleles()
                .iter()
                .position(|&a| a == a1.replace("-", "").as_bytes())
                .unwrap() as i32;
            let i2 = record
                .alleles()
                .iter()
                .position(|&a| a == a2.replace("-", "").as_bytes())
                .unwrap() as i32;
...
fn phase_variants(
    matrix: &Vec<BTreeMap<usize, (String, u8)>>,
) -> (
    HashSet<usize>,
    HashSet<usize>,
    Vec<Option<String>>,
    ...
    let (p1, p2, r1, r2) = skydive::wmec::phase_all(&wmec_matrix, 40, 20);

    // Create sets to store read indices for each haplotype
    let mut h1_reads = HashSet::new();
    let mut h2_reads = HashSet::new();

    // For each read, count how many variants match each haplotype
    for (read_index, read) in wmec_matrix.reads.iter().enumerate() {
        let mut num_p1 = 0;
        let mut num_p2 = 0;
        ...
        for i in 0..p1.len() {
            if read[i].is_some() {
                total_variants += 1;
                if read[i] == Some(p1[i]) && read[i] != Some(p2[i]) {
                    num_p1 += 1;
                } else if read[i] == Some(p2[i]) && read[i] != Some(p1[i]) {
                    num_p2 += 1;
                }
        ...
        // Only assign read if it has enough informative variants and clear preference
        if total_variants >= 2 {
            // Adjust this threshold as needed
            if num_p1 > num_p2 {
                h1_reads.insert(read_index);
            } else if num_p2 > num_p1 {
                h2_reads.insert(read_index);
            }
            // Reads with equal matches to both haplotypes are left unassigned
        }
    }
